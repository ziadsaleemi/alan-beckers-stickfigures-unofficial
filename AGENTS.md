...
- Role-play sprite frames are generated by `script/generate_roleplay_sprites.py`. The script writes `role_*.png` files into each `img/<Stickfigure>/` folder and injects `Role...` action clips between `BEGIN ABS_ROLEPLAY_SPRITES` / `END ABS_ROLEPLAY_SPRITES` markers in each character `actions.xml`. The Swift renderer should prefer these `Role...` clips and keep existing stock clips as fallbacks.
