...
- Native macOS Local AI behavior is optional and uses Ollama's local HTTP API (`/api/tags`, `/api/chat`) from `macos/AlanBeckersStickfiguresLauncher/main.swift`. The settings UI stores `localAIEnabled`, `ollamaBaseURL`, and `ollamaModel` in user defaults, filters out embedding-only models, offers only sprite-backed actions supported by each character, and sends only character/action metadata plus sanitized desktop context to the model. AI/story behavior is driven by spaced-out `StoryBeat` scenes, pauses per-character AI suggestions while a scene is active, throttles one-off model cues between scenes, and includes solo actions plus dynamic two-character composite interactions such as followPartner, copyPartner, guardPartner, ambush, playChase, spar, tease, celebrate, and observePartner, biased by simple lore affinities.
