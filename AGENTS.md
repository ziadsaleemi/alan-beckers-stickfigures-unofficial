...
- Role-play sprite frames are generated by `script/generate_roleplay_sprites.py`. The script writes `role_*.png` files into each `img/<Stickfigure>/` folder and injects `Role...` action clips between `BEGIN ABS_ROLEPLAY_SPRITES` / `END ABS_ROLEPLAY_SPRITES` markers in each character `actions.xml`. Role sprites must be source-derived copies of existing character artwork, not custom-drawn stick figures, so proportions, line weight, head style, colors, anchors, and sprite dimensions match the original image set. Keep the per-character overrides that avoid prop-heavy or effect-heavy source frames for generic role actions. The generated role set is 28 clips / 56 frames per character / 504 PNGs total and includes chase lead/follow, spar attack/block, tease/reaction, rescue give/receive, trade-place, celebration, and observation clips. The Swift renderer should prefer these `Role...` clips and keep existing stock clips as fallbacks.
